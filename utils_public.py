...
def onehot_and_flatten(grids):
    """
    Convert a 2D array of grids to a 2D array of one-hot encoded grids.
    :param grids:
        A 2D array of grids, where each row is a grid.
    :return:
...
def load_grids():
    """
    Load the grids from the dataset. Grids are stored in 5 separate files due to Github file size limits.
    :return:
        A 2D array of grids, where each row is a grid.
    """
    grids = []
...
def select_rated_subset(grids, ratings):
    """
    Select the subset of grids and ratings that have been rated.
    :param grids:
        A 2D array of grids, where each row is a grid.
    :param ratings:
        A 2D array of ratings, where each row is a grid and each column is an advisor.
    :return:
        A tuple of the subset of grids and ratings that have been rated.
    """
    mask = np.where(~np.isnan(ratings))
    return grids[mask], ratings[mask]

def plot_ratings_histogram(ratings, withmin=False):
    """
    Plot the histogram of ratings for each category.
    :param ratings:
        A 2D array of ratings, where each row is a grid and each column is an advisor.
    """
    fig, axes = plt.subplots(1,4+withmin, figsize = (16,3))
    fig.tight_layout()
    ...
    for i in range(4):
        ...
        axis.set_xlabel(f"{names[i]} Score")
    if withmin:
        min_ratings = np.min(ratings, axis=1)
        axis=axes[4]
        axis.hist(min_ratings, bins=20)
        axis.spines['right'].set_visible(False)
        axis.spines['top'].set_visible(False)
        axis.set_xlabel("Min Score")
    plt.show()
...
def plot_grid_image(grid, on_ax=None):
    ...
    :param grid:
        A 7x7 grid of integers representing the city
    :param on_ax:
...
def plot_districts():
    """
    Show the five district sprites in a row with captions
    """
...
def plot_n_grids(grids):
    """
    Plot an array of grids. Infers number based on shape of grids
    :param grids:
        An n x w x w array of city grids.
    """
    n = len(grids)
